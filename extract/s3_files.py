...
boto3_session=boto3.Session(aws_access_key_id=access_key_id,aws_secret_access_key=secret_key)
...
def upload_files():
    ...
    date=datetime.now()
    filename=f'{date.year}/{date.month}/{date.day}/daily_mix.csv'
   
    response= s3_resource.Object("data-pipline-etl",filename).upload_file("../data/Discover_Weekly.csv")
